...
class Car(tk.Tk, object):
    ...
    def _build_plot(self):
        ...
        # MG1 Map 
        self.mg1_map = plt.subplot2grid((5, 5), (0, 1))
        # W_mg2 
        self.w_mg1 = plt.subplot2grid((5, 5), (1, 1))
        # T_mg2 
        self.t_mg1 = plt.subplot2grid((5, 5), (2, 1))
        # kW_mg2 
        self.fig_kW_mg1 = plt.subplot2grid((5, 5), (3, 1))
        # Battery Charge
        self.fig_battery_charge = plt.subplot2grid((5, 5), (4, 1))
    ...
    def reset(self,current_cycle_target_speed,next_cycle_target_speed):
        ...
        s = np.array([self.W_mg1,self.T_mg1,self.W_mg2,self.T_mg2,self.T_brake,self.soc,
    ...
    def vehicle_powertrain(self,action,time,cycle_end_time,next_cycle_target_speed,next2_cycle_target_speed):
        ...
        if action == 0:   # no action
            ...
            if abs(self.W_mg1) > max(self.mg1_map_spd):
                self.T_mg1 = 0
        elif action == 5: # increase Motor torque
            if self.T_mg2 < 0:
                self.T_mg2 = 0
            ...
            if abs(self.W_mg2) > max(self.mg2_map_spd):
                self.T_mg2 = 0
        elif action == 6: # decrease Motor torque
            if self.T_mg2 < 0:
                self.T_mg2 = 0
            ...
            if abs(self.W_mg2) > max(self.mg2_map_spd):
                self.T_mg2 = 0
        elif action == 7: # increase Generator torque
            if self.T_mg2 > 0:
                self.T_mg2 = 0
            ...
            if abs(self.W_mg2) > max(self.mg2_map_spd):
                self.T_mg2 = 0
        elif action == 8: # decrease Generator torque
            if self.T_mg2 > 0:
                self.T_mg2 = 0
            if self.T_mg2+self.d_T_mg2 > 0:
                self.mg2 = 0
        ...
        if self.W_wheel<0 or self.W_mg1<0 or self.W_mg2<0 or self.v_vehicle<0 or self.v_vehicle_kmh<0: 
            self.W_wheel = 0
            self.W_mg1 = 0
            self.W_mg2 = 0
            self.v_vehicle = 0
        ...
        self.W_batt_c = self.kW_mg1/self.mg1_eff*1000 + self.kW_mg2/self.mg2_eff*1000 # +: discharge; -: charge 
        ...
        s_ = np.array([self.W_mg1,self.T_mg1,self.W_mg2,self.T_mg2,self.T_brake,self.soc,
