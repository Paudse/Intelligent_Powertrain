...
class Car(tk.Tk, object):
    ...
    def reset(self,current_cycle_target_speed,next_cycle_target_speed):
        ...
        s = np.array([self.W_mg2,self.T_mg2,self.i_gear,self.T_brake,self.soc,
    ...
    def vehicle_powertrain(self,action,time,cycle_end_time,next_cycle_target_speed,next2_cycle_target_speed):
        ...
        self.i_gear_ratio_map = np.array([1.5, 1/1.5]) # 2008 Saab
        ...
        if action == 0:   # no action
            ...
            if abs(self.W_mg2) > max(self.mg2_map_spd):
                self.T_mg2 = 0
        elif action == 5: # shift up one gear 
            ...
            if abs(self.W_mg2) > max(self.mg2_map_spd):
                self.T_mg2 = 0
        elif action == 6: # shift down one gear 
            ...
            if abs(self.W_mg2) > max(self.mg2_map_spd):
                self.T_mg2 = 0
        elif action == 7: # increase Brake torque
            ...
            if abs(self.W_mg2) > max(self.mg2_map_spd):
                self.T_mg2 = 0
        elif action == 8: # decrease Brake torque
        ...
        ### Powertrain Dynamics ###
        self.dW_wheel = \
        (self.T_mg2*self.i_gear_ratio*self.i_f_gear_ratio \
        -self.T_brake \
        -self.total_resis_torque) \
        /((self.r_wheel**2*self.m_equ_vehicle_kg) \
        +self.I_mg2*(self.i_gear_ratio**2)*(self.i_f_gear_ratio**2))

        # calculate speed
        self.W_wheel = self.W_wheel + self.dW_wheel # rad/s
        self.W_mg2 = self.W_wheel*self.i_gear_ratio*self.i_f_gear_ratio
        ...
        s_ = np.array([self.W_mg2,self.T_mg2,self.i_gear,self.T_brake,self.soc,
