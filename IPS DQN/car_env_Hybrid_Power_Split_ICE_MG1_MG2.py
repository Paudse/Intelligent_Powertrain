...
class Car(tk.Tk, object):
    def __init__(self,time,cycle_time,cycle_speed,cycle_round,cycle_dt,cycle_total_step_number,vehicle_name,
        ...
        self.mg1_map_spd = mg1_map_spd
        self.mg1_map_trq = mg1_map_trq
        self.mg1_eff_map = mg1_eff_map
        self.mg1_max_trq = mg1_max_trq
        self.mg1_max_gen_trq = mg1_max_gen_trq
    ...
    def _build_plot(self):
        ...
        # MG1 Map 
        self.mg1_map = plt.subplot2grid((5, 5), (0, 2))
        # W_mg2 
        self.w_mg1 = plt.subplot2grid((5, 5), (1, 2))
        # T_mg2 
        self.t_mg1 = plt.subplot2grid((5, 5), (2, 2))
        # kW_mg2 
        self.fig_kW_mg1 = plt.subplot2grid((5, 5), (3, 2))
        # Battery Charge
        self.fig_battery_charge = plt.subplot2grid((5, 5), (4, 2)) 
        
        # MG2 Map 
        self.mg2_map = plt.subplot2grid((5, 5), (0, 3))
        # W_mg2 
        self.w_mg2 = plt.subplot2grid((5, 5), (1, 3))
        # T_mg2 
        self.t_mg2 = plt.subplot2grid((5, 5), (2, 3))
        # kW_mg2 
        self.fig_kW_mg2 = plt.subplot2grid((5, 5), (3, 3))
        # Battery Charge
        # self.fig_battery_charge = plt.subplot2grid((5, 5), (4, 3))

        # Battery SOC
        self.fig_battery_soc = plt.subplot2grid((5, 5), (4, 3))
        
        # Instant Reward
        self.fig_reward = plt.subplot2grid((5, 5), (4, 4))
    ...
    def reset(self,current_cycle_target_speed,next_cycle_target_speed):
        ...
        s = np.array([self.W_ice,self.T_ice,self.W_mg1,self.T_mg1,self.W_mg2,self.T_mg2,self.T_brake,self.soc,
    ...
    def cycle_plot(self,cycle_time,cycle_speed,cycle_name,cycle_round,last_time_iteration,max_cycle_reward):
        ...
        #################################################################################
        ###########################
        X,Y = np.meshgrid(self.mg1_map_spd*rads2rpm, self.mg1_map_trq)
        C_eff = self.mg1_map.contour(X, Y, self.mg1_eff_map.T, 10, alpha=.75, cmap=plt.cm.hot)
        for ii in range(len(self.T_mg1_array)):
            if self.T_mg1_array[ii] < 0:
                self.mg1_map.plot(self.W_mg1_array[ii]*rads2rpm,self.T_mg1_array[ii],"o",color='purple',markersize=2 )
            else:
                self.mg1_map.plot(self.W_mg1_array[ii]*rads2rpm,self.T_mg1_array[ii],"o",color='C2',markersize=2 )
        self.mg1_map.plot(self.mg1_map_spd*rads2rpm, self.mg1_max_trq,'--',color='red',linewidth=3)
        self.mg1_map.plot(self.mg1_map_spd*rads2rpm, self.mg1_max_gen_trq,'--',color='red',linewidth=3)
        self.mg1_map.clabel(C_eff, inline=False, fontsize=set_tick_fontsize) 

        self.mg1_map.set_title('MG1 Efficiency',fontsize=set_title_fontsize)
        self.mg1_map.set_xlabel('rpm',fontsize=set_label_fontsize)
        self.mg1_map.set_ylabel('Nm',fontsize=set_label_fontsize)
        self.mg1_map.tick_params(labelsize=set_tick_fontsize)
        self.mg1_map.grid(color='gray', linestyle='-', linewidth=0.5)

        ###########################
        self.w_mg1.plot(self.time_array,[i * rads2rpm for i in self.W_mg1_array],"o",color='C1',markersize=2 )

        self.w_mg1.set_title('MG1 Speed',fontsize=set_title_fontsize)
        self.w_mg1.set_xlabel('',fontsize=set_label_fontsize)
        self.w_mg1.set_ylabel('rpm',fontsize=set_label_fontsize)
        self.w_mg1.tick_params(labelsize=set_tick_fontsize)
        self.w_mg1.set(xlim=[0, max(cycle_time)])
        self.w_mg1.grid(color='gray', linestyle='-', linewidth=0.5)

        ###########################
        self.t_mg1.plot(self.time_array,np.minimum(self.T_mg1_array, 0),"o",color='purple',markersize=2 )
        self.t_mg1.plot(self.time_array,np.maximum(self.T_mg1_array, 0),"o",color='C1',markersize=2 )
        self.t_mg1.plot([0, max(cycle_time)], [0, 0],'black',linewidth=2)

        self.t_mg1.set_title('MG1 Torque',fontsize=set_title_fontsize)
        self.t_mg1.set_xlabel('',fontsize=set_label_fontsize)
        self.t_mg1.set_ylabel('Nm',fontsize=set_label_fontsize)
        self.t_mg1.tick_params(labelsize=set_tick_fontsize)
        self.t_mg1.set(xlim=[0, max(cycle_time)])
        self.t_mg1.grid(color='gray', linestyle='-', linewidth=0.5)

        ###########################
        self.fig_kW_mg1.plot(self.time_array,np.minimum(self.kW_mg1_array, 0),"o",color='purple',markersize=2 )
        self.fig_kW_mg1.plot(self.time_array,np.maximum(self.kW_mg1_array, 0),"o",color='C1',markersize=2 )
        self.fig_kW_mg1.plot([0, max(cycle_time)], [0, 0],'black',linewidth=2)

        self.fig_kW_mg1.set_title('MG1 Power',fontsize=set_title_fontsize)
        self.fig_kW_mg1.set_xlabel('',fontsize=set_label_fontsize)
        self.fig_kW_mg1.set_ylabel('kW',fontsize=set_label_fontsize)
        self.fig_kW_mg1.tick_params(labelsize=set_tick_fontsize)
        self.fig_kW_mg1.set(xlim=[0, max(cycle_time)])
        self.fig_kW_mg1.grid(color='gray', linestyle='-', linewidth=0.5)
        ...
        scipy.io.savemat('result_data/'+self.vehicle_name+'/iteration_'+str(last_time_iteration+cycle_round)+'_'+str(cycle_name)+'_reward-'+str(round(max_cycle_reward,2))+'.mat', {
            ...
            'sim_W_mg1_dqn':self.W_mg1_array,
            'sim_T_mg1_dqn':self.T_mg1_array,
            'sim_P_mg1_dqn':self.kW_mg1_array,
    ...
    def vehicle_powertrain(self,action,time,cycle_end_time,next_cycle_target_speed,next2_cycle_target_speed):
        ...
        self.i_mg2_gear_ratio = 1 # 2.63 # fixed ratio on MG2 in 2010 Prius (mentioned in Namwook Kim's journal paper)
        ...
        self.d_T_mg1 = 20
        self.T_mg1_max = np.interp(self.W_mg1, self.mg1_map_spd, self.mg1_max_trq)
        self.T_mg1_min = np.interp(self.W_mg1, self.mg1_map_spd, self.mg1_max_gen_trq)
        self.I_mg1 = 0.0000226
        ...
        I_v = self.m_equ_vehicle_kg*self.r_wheel**2/self.i_f_gear_ratio**2 # vehicle inertia 

        # ================================================================== 
        # Prius configuration
        D = [R+S, -R, -S]
        # Mode 4: split mode
        D_mode4 = [[self.I_ice +I_c, 0,                0,               D[0]], # engine (carrier gear)
                   [0,       I_v+self.I_mg2+I_r ,      0,               D[1]], # wheel & mg2 (ring gear)
                   [0,       0,                        self.I_mg1+I_s,  D[2]], # mg1 (sun gear)
                   [D[0],    D[1],                     D[2],             0]]
        AA = np.linalg.inv(D_mode4)
        ...
        if action == 0:   # no action
            if self.W_ice > max(self.fc_map_spd):
                self.T_ice = 0
            if abs(self.W_mg1) > max(self.mg1_map_spd):
                self.T_mg1 = 0
            ...
            if abs(self.W_mg2) > max(self.mg2_map_spd):
                self.T_mg2 = 0
        elif action == 7: # increase Motor torque
            if self.T_mg1 < 0:
                self.T_mg1 = 0
            if self.T_mg1+self.d_T_mg1 > self.T_mg1_max:
                self.mg1 = self.T_mg1_max
            else: 
                self.T_mg1 = self.T_mg1+self.d_T_mg1
            self.T_brake = 0
            if self.W_ice > max(self.fc_map_spd):
                self.T_ice = 0
            if abs(self.W_mg1) > max(self.mg1_map_spd):
            ...
            if abs(self.W_mg2) > max(self.mg2_map_spd):
                self.T_mg2 = 0
        elif action == 8: # decrease Motor torque
            if self.T_mg1 < 0:
                self.T_mg1 = 0
            if self.T_mg1-self.d_T_mg1 < 0:
                self.T_mg1 = 0
            else: 
                self.T_mg1 = self.T_mg1-self.d_T_mg1
            self.T_brake = 0
            ...
            if abs(self.W_mg2) > max(self.mg2_map_spd):
                self.T_mg2 = 0
        elif action == 9: # increase Generator torque
            if self.T_mg1 > 0:
                self.T_mg1 = 0
            if self.T_mg1-self.d_T_mg1 < self.T_mg1_min:
                self.T_mg1 = self.T_mg1_min
            else: 
                self.T_mg1 = self.T_mg1-self.d_T_mg1
            if self.W_ice > max(self.fc_map_spd):
                self.T_ice = 0
            ...
            if abs(self.W_mg2) > max(self.mg2_map_spd):
                self.T_mg2 = 0
        elif action == 10: # decrease Generator torque
            if self.T_mg1 > 0:
                self.T_mg1 = 0
            if self.T_mg1+self.d_T_mg1 > 0:
                self.mg1 = 0
            ...
            if abs(self.W_mg2) > max(self.mg2_map_spd):
                self.T_mg2 = 0
        elif action == 11: # increase Brake torque
            ...
            if abs(self.W_mg2) > max(self.mg2_map_spd):
                self.T_mg2 = 0
        elif action == 12: # decrease Brake torque
        ...
        if self.T_mg1 > self.T_mg1_max:
            self.T_mg1 = self.T_mg1_max
            punishment = punishment + 10
        if self.T_mg1 < self.T_mg1_min:
            self.T_mg1 = 0
            punishment = punishment + 10            
        if self.W_mg1 > max(self.mg1_map_spd):
            self.T_mg1 = 0
            punishment = punishment + 10
        if self.W_mg1 < 0:
            self.T_mg1 = 0
            punishment = punishment + 10
        if self.soc < self.soc_min and self.T_mg1 > 0:
            self.T_mg1 = 0
            punishment = punishment + 10
        if self.soc > self.soc_max and self.T_mg1 < 0:
            self.T_mg1 = 0
            punishment = punishment + 10
        ...
        ### Powertrain Dynamics ###
        dW_ice = AA[0][0]*self.T_ice + AA[0][1]*(self.T_mg2*self.i_mg2_gear_ratio - self.total_resis_torque/self.i_f_gear_ratio) + AA[0][2]*self.T_mg1
        dW_out = AA[1][0]*self.T_ice + AA[1][1]*(self.T_mg2*self.i_mg2_gear_ratio - self.total_resis_torque/self.i_f_gear_ratio) + AA[1][2]*self.T_mg1

        # calculate speed
        self.W_ice = self.W_ice + dW_ice
        self.W_out = self.W_out + dW_out
        self.W_wheel = self.W_out / self.i_f_gear_ratio # rad/s
        self.W_mg2 = self.W_out * self.i_mg2_gear_ratio
        self.W_mg1 = -(D[0]*self.W_ice+D[1]*self.W_out)/D[2]
        ...
        self.kW_mg1 = self.W_mg1 * self.T_mg1/1000
        ...
        # calculate mg1 efficiency
        mg1_map = interpolate.interp2d(self.mg1_map_trq,self.mg1_map_spd,self.mg1_eff_map,  kind='cubic')
        self.mg1_eff = mg1_map(self.T_mg1, self.W_mg1)[0]
        ...
        self.W_batt_c = self.kW_mg2/self.mg2_eff*1000 + self.kW_mg2/self.mg2_eff*1000 # +: discharge; -: charge 
        ...
        self.W_mg1_array.append(self.W_mg1)
        self.T_mg1_array.append(self.T_mg1)
        self.kW_mg1_array.append(self.kW_mg1)
        ...
        s_ = np.array([self.W_ice,self.T_ice,self.W_mg1,self.T_mg1,self.W_mg2,self.T_mg2,self.T_brake,self.soc,
