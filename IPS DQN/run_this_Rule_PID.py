...
# set_reward_decay = 0.9
# set_e_greedy = 0.9
# set_replace_target_iter = 10
...
vehicle_name = 'Hybrid_ICE_5AT_MG2_Rule_PID'
...
##    de = 1/cycle_dt
    for step in range(999999999999999):
        ...
        current_v_vehicle_kmh = observation[7]
        # print(current_v_vehicle_kmh)
        ...
        while True:
            ...
            T_demand = PID1.control_command(current_cycle_target_speed,current_v_vehicle_kmh)

            # RL take action and get next observation and reward
##            observation_, reward, done = env.step(action)
            observation_, reward, last_iteration, latest_reward  = env.vehicle_powertrain(T_demand,time,cycle_end_time,next_cycle_target_speed,next2_cycle_target_speed)
        ...
        if time == cycle_time[-1]: # clean figure for next cycle 
          last_time_iteration = 0
          max_cycle_reward = latest_reward[-1]

          env.clear_figure()
          print('Iteration:', cycle_round, 'cycle_reward = ',latest_reward[-1])
...
if __name__ == "__main__":
    ...
    if vehicle_type == 'car_env_NV_ICE_5AT':
        ...
        env = Car(time,cycle_time,cycle_speed,cycle_round,cycle_dt,cycle_total_step_number,vehicle_name,
                  fc_map_spd,fc_map_trq,fc_fuel_map,fc_max_trq,min_fuel_consum_line_spd,min_fuel_consum_line_trq,
                  mg2_map_spd,mg2_map_trq,mg2_eff_map,mg2_max_trq,mg2_max_gen_trq)
    elif vehicle_type == 'car_env_Hybrid_ICE_5AT_MG2_Rule_PID':
        from car_env_Hybrid_ICE_5AT_MG2_Rule_PID import Car
        env = Car(time,cycle_time,cycle_speed,cycle_round,cycle_dt,cycle_total_step_number,vehicle_name,
                  fc_map_spd,fc_map_trq,fc_fuel_map,fc_max_trq,min_fuel_consum_line_spd,min_fuel_consum_line_trq,
                  mg2_map_spd,mg2_map_trq,mg2_eff_map,mg2_max_trq,mg2_max_gen_trq)
