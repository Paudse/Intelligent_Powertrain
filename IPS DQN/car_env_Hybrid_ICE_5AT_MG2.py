...
class Car(tk.Tk, object):
    ...
    def reset(self,current_cycle_target_speed,next_cycle_target_speed):
        ...
        # Normalization
        # print(max(self.fc_map_spd),max(self.fc_map_trq),max(self.mg2_map_spd),max(self.mg2_map_trq),self.i_gear_max,self.T_brake_max,120)
        s = np.array([self.W_ice/max(self.fc_map_spd),
            self.T_ice/max(self.fc_map_trq),
            self.W_mg2/max(self.mg2_map_spd),
            self.T_mg2/max(self.mg2_map_trq)/2+0.5,
            self.i_gear/self.i_gear_max,
            self.T_brake/self.T_brake_max,
            self.soc/1,
            self.v_vehicle_kmh/140,
            current_cycle_target_speed/140,
            next_cycle_target_speed/140])
    ...
    def vehicle_powertrain(self,action,time,cycle_end_time,next_cycle_target_speed,next2_cycle_target_speed):
        ...
        if action == 7 or action == 8:
            punishment = punishment + 100 # less gear shift
        ...
        # Normalization
        s_ = np.array([self.W_ice/max(self.fc_map_spd),
            self.T_ice/max(self.fc_map_trq),
            self.W_mg2/max(self.mg2_map_spd),
            self.T_mg2/max(self.mg2_map_trq)/2+0.5,
            self.i_gear/self.i_gear_max,
