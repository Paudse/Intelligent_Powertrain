...
class Car(tk.Tk, object):
    ...
    def reset(self,current_cycle_target_speed,next_cycle_target_speed):
        ...
        s = np.array([self.W_ice,self.T_ice,self.i_CVT_ratio,self.T_brake,
    ...
    def cycle_plot(self,cycle_time,cycle_speed,cycle_name,cycle_round,last_time_iteration,max_cycle_reward):
        ...
        ###########################
        self.gear.plot([0, max(cycle_time)], [self.i_CVT_ratio_min, self.i_CVT_ratio_min],'c--')
        self.gear.plot([0, max(cycle_time)], [self.i_CVT_ratio_max, self.i_CVT_ratio_max],'c--')
        self.gear.plot(self.time_array,self.i_CVT_ratio_array,"o",color='C1',markersize=2 )
        ...
        self.gear.set(xlim=[0, max(cycle_time)],ylim=[self.i_CVT_ratio_min-0.2, self.i_CVT_ratio_max+0.2])
    ...
    def vehicle_powertrain(self,action,time,cycle_end_time,next_cycle_target_speed,next2_cycle_target_speed):
        ...
        if action == 0:   # no action
            ...
            if self.W_ice > max(self.fc_map_spd):
                self.T_ice = 0
            # elif self.W_ice < self.W_ice_idle_spd_rs:
            #     self.T_ice = 0
            self.T_brake = 0
            ...
            if self.W_ice > max(self.fc_map_spd):
                self.T_ice = 0
        elif action == 3: # shift up one gear 
            ...
            if self.W_ice > max(self.fc_map_spd):
                self.T_ice = 0
        elif action == 4: # shift down one gear 
        ...
        if self.W_wheel<0 or self.W_ice<0 or self.v_vehicle<0 or self.v_vehicle_kmh<0: 
            self.W_wheel = 0
            self.W_ice = 0
            self.v_vehicle = 0
            self.v_vehicle_kmh = 0
        ...
        reward = (1/1)*(50*0.95**(abs(self.current_cycle_target_speed-self.v_vehicle_kmh))  \
        +40*0.5**(self.ICE_fuel_consum) \
        +10*0.97**(punishment))
        ...
        s_ = np.array([self.W_ice,self.T_ice,self.i_CVT_ratio,self.T_brake,
